# -*- coding: utf-8 -*-
"""
Training script for pretrained Task-Aware Video DeepJSCC with frozen TSM.

PRETRAINED INITIALIZATION VERSION (28 August 2026)
This file requires ``--videojscc_ckpt`` and loads the matching reconstruction-
only VideoJSCC ``jscc.*`` and ``temporal.*`` weights before joint training.
...
from model.ta_video_jscc_tsm_pretrained import TAVideoJSCC
...
def train_epoch(model, optimizer, device, data_loader, max_iters=None):
    ...
    totals  = {'loss': 0, 'l_task': 0, 'l_recon': 0,
               'l_rate': 0, 'weighted_task': 0, 'weighted_recon': 0,
               'weighted_rate': 0, 'rate_mean': 0, 'score_mean': 0,
...
def load_videojscc_initialization(model, checkpoint_path):
    """Strictly load jscc.* and temporal.* from a VideoJSCC checkpoint."""
    if not checkpoint_path:
        raise ValueError('--videojscc_ckpt is required for joint training')
    ...
    jscc_state = {
        key[len('jscc.'):]: value for key, value in state.items()
        if key.startswith('jscc.')
    }
    temporal_state = {
        key[len('temporal.'):]: value for key, value in state.items()
        if key.startswith('temporal.')
    }
    if not jscc_state or not temporal_state:
        raise KeyError(
            "Expected checkpoint keys starting with 'jscc.' and 'temporal.'. "
            'A frame-only DeepJSCC checkpoint is not sufficient.'
        )
...
@torch.no_grad()
def evaluate_pretrained_videojscc(model, loader, device):
    """Evaluate the loaded reconstruction model before any optimizer step."""
    model.eval()
    mse_sum = 0.0
    correct = 0
    total = 0
    for gops, labels in tqdm(
        loader, desc='initial VideoJSCC check', leave=False
    ):
        gops = gops.to(device, non_blocking=True)
        labels = labels.to(device, non_blocking=True)
        reconstructed = model.forward_no_selection(gops)
        mse = torch.nn.functional.mse_loss(reconstructed, gops)
        mse_sum += mse.item() * labels.size(0)
        if model.tsn is not None:
    ...
    return {
        'mse': mean_mse,
        'psnr': -10.0 * np.log10(max(mean_mse, 1e-12)),
...
def config_parser():
    ...
    # Frozen downstream action-recognition model
    parser.add_argument('--task_model', choices=['tsm', 'tsn', 'none'],
                        default='tsm',
                        help='Frozen task network used for task loss. TSM is '
                             'the recommended temporally interactive model; '
                             'TSN preserves the historical baseline.')
    parser.add_argument('--tsm_ckpt',
                        default='downstream/action_recognition/weights/'
                                'tsm_ucf101_5f_split_best.pth',
                        help='Full UCF101 TSM checkpoint produced by '
                             'finetune_tsm_split.py')
    parser.add_argument('--tsn_head_ckpt',
                        default='downstream/action_recognition/weights/tsn_ucf101_head.pth')
    parser.add_argument(
        '--videojscc_ckpt', default=None,
        help='Matching reconstruction-only VideoJSCC best.pkl. Loads the '
             'jscc and temporal weights before joint optimization.',
    )
...
def train_pipeline(params):
    ...
    # Initialize reconstruction from the matching VideoJSCC baseline before
    # activating task-guided joint optimization.
    ...
    print(
        f"Trainable TA-VideoJSCC params: {trainable:,} | "
        f"Frozen task-model params: {frozen:,}"
    )
    ...
    # Establish the no-update baseline with selection bypassed. This should
    # reproduce the matching VideoJSCC checkpoint before the first backward.
    initial_metrics = evaluate_pretrained_videojscc(model, test_loader, device)
    print(
        "\nInitial no-update VideoJSCC check (selector bypassed)\n"
        f"  PSNR:      {initial_metrics['psnr']:.2f} dB\n"
        f"  TSM Top-1: {initial_metrics['top1']:.2f}%\n"
        f"  samples:   {initial_metrics['samples']}\n"
    )
    for key, value in initial_metrics.items():
        writer.add_scalar(f'initial/{key}', value, 0)
    with open(os.path.join(root_config_dir, 'initial_metrics.json'),
              'w', encoding='utf-8') as handle:
        json.dump(initial_metrics, handle, indent=2)
