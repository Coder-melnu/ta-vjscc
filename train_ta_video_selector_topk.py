# -*- coding: utf-8 -*-
"""Train exact fixed-budget top-k Task-Aware VideoJSCC on locked UCF101.

The reconstruction-only VideoJSCC+TFM checkpoint is the warm start. The
selector is active from epoch 1 and all trainable codec/TFM/selector parameters
are optimized with the same joint loss for the full run. The locked TSN remains
frozen and in evaluation mode while gradients flow through it to reconstructed
frames.
...
Reference recipe:
    python train_ta_video_selector_joint.py \
        --channel AWGN --snr_list 13 --ratio_list 1/6 \
        --epochs 25 --lambda_task 0.001 --lambda_recon 1.0 \
        --lambda_rate 0 --tau 1.0 --tau_min 0.3 \
        --init_lr 1e-5 --videojscc_ckpt /path/to/best.pt
"""
...
from model.ta_video_jscc_selector_topk import TAVideoJSCC
...
def fixed_rng(device, seed):
    """Use fixed channel-noise draws without perturbing training RNG state."""
...
def evaluate_epoch(model, device, data_loader, channel_seed):
    ...
    with fixed_rng(device, channel_seed), torch.no_grad():
        for gops, labels in data_loader:
            gops   = gops.to(device)
...
def load_videojscc_initialization(model, checkpoint_path):
    ...
    if isinstance(artifact, dict):
        checkpoint_manifest_hash = artifact.get('split_manifest_sha256')
        if checkpoint_manifest_hash is None and isinstance(artifact.get('config'), dict):
            checkpoint_manifest_hash = artifact['config'].get('split_manifest_sha256')
    ...
    with open(checkpoint_path, 'rb') as handle:
        for block in iter(lambda: handle.read(1024 * 1024), b''):
            checkpoint_sha256.update(block)
    return {
        'checkpoint_sha256': checkpoint_sha256.hexdigest(),
        'split_manifest_sha256': checkpoint_manifest_hash,
    }
...
def audit_topk_gradient(model, loader, device, seed=12345):
    """Verify exact forward budget and nonzero scorer gradient before training."""
    model.train()
    model.tsn.eval()
    model.set_selector_mode('learned')
    ...
    if device.type == 'cuda':
        torch.cuda.manual_seed_all(seed)
    loss, _, _, _ = model.joint_loss(gops, labels)
    loss.backward()
    final_layer = model.selector.scorer.net[-1]
    gradient_sum = sum(
        parameter.grad.detach().abs().sum().item()
        for parameter in final_layer.parameters()
        if parameter.grad is not None
    )
    mask_mean = model.last_power_audit['mask_mean']
    if abs(mask_mean - model.keep_fraction) > 1e-7:
        raise RuntimeError(
            f'Exact top-k budget failed: {mask_mean} != {model.keep_fraction}'
        )
    if not math.isfinite(gradient_sum) or gradient_sum <= 0.0:
        raise RuntimeError('Selector received no finite nonzero gradient')
    for parameter in model.parameters():
        parameter.grad = None
    result = {
        'exact_mask_mean': mask_mean,
        'keep_count': model.last_power_audit['keep_count'],
        'available_locations': model.last_power_audit['locations'],
        'selector_final_layer_gradient_l1': gradient_sum,
    }
    print('\nExact top-k/STE gradient audit')
    for key, value in result.items():
        print(f'  {key}: {value}')
    print('  PASS: exact forward budget and selector gradient verified')
    return result
...
def config_parser():
    parser = argparse.ArgumentParser(
        ...
        description='Directly joint-fine-tune TA-VideoJSCC with selector active'
    )
    ...
    parser.add_argument('--tau',           type=float, default=1.0,
                        help='Initial soft-selector temperature')
    parser.add_argument('--tau_min',       type=float, default=0.3,
                        help='Final temperature after linear annealing')
    parser.add_argument('--keep_fraction', type=float, default=0.5,
                        help='Exact spatial latent fraction transmitted per frame')
    ...
    parser.add_argument('--videojscc_ckpt', required=True,
                        help='Matching clean VideoJSCC best.pt initialization')
    ...
    parser.add_argument('--lambda_rate',   type=float, default=0.0,
                        help='Must remain zero: exact top-k already fixes rate')
    ...
    parser.add_argument('--max_time',      type=float, default=0.0,
                        help='Max wall-clock training time in hours; 0 disables')
    parser.add_argument('--seed',          type=int,   default=42)
    parser.add_argument('--split_seed',    type=int,   default=42,
                        help='Fixed dataset split seed; keep at 42 for the locked protocol')
    parser.add_argument('--val_channel_seed', type=int, default=1042,
                        help='Fixed validation channel-noise seed')
...
def train_pipeline(params):
    ...
    if params['lambda_rate'] != 0.0:
        raise ValueError('Top-k fixes the rate exactly; use --lambda_rate 0')
    ...
    print(f"λ_task={params['lambda_task']} | λ_recon={params['lambda_recon']} | "
          f"λ_rate={params['lambda_rate']} | "
          f"τ={params['tau']} → {params['tau_min']}")
    print(f"Direct joint fine-tuning: {params['epochs']} epochs")
    ...
    baseline_metadata = load_videojscc_initialization(model, params['videojscc_ckpt'])
    baseline_manifest_hash = baseline_metadata['split_manifest_sha256']
    if baseline_manifest_hash is None:
        raise RuntimeError('Baseline checkpoint does not record a split-manifest hash')
    if baseline_manifest_hash != split_manifest_hash:
        raise RuntimeError(
            f'Baseline/evaluation split mismatch: {baseline_manifest_hash} != {split_manifest_hash}'
        )
    tsn_manifest_hash = model.tsn.checkpoint_metadata.get('split_manifest_sha256')
    if tsn_manifest_hash != split_manifest_hash:
        raise RuntimeError(
            f'TSN/evaluation split mismatch: {tsn_manifest_hash} != {split_manifest_hash}'
        )
    print(f'[Split audit] PASS: {split_manifest_hash}')
    model.set_selector_mode('learned')
    model.set_temperature(params['tau'])

    audit = audit_all_ones_selector(model, val_loader, device)
    model.set_selector_mode('learned')
    topk_audit = audit_topk_gradient(model, train_loader, device)
    ...
    tag = (f"TAVideoJSCC_{params['channel']}_c{c}"
           ...
           f"_topk{params['keep_fraction']:.2f}"
           f"_joint{params['epochs']}ep"
    ...
    # TSN is frozen; codec, TFM and selector are jointly trainable.
    ...
    try:
        with tqdm(range(params['epochs']), disable=params['disable_tqdm']) as t:
            for epoch in t:
                ...
                writer.add_scalar(
                    'val/actual_cbr',
                    params['ratio'] * params['keep_fraction'], epoch,
                )
                ...
                ckpt_path = os.path.join(root_ckpt_dir, f'epoch_{epoch}.pt')
                torch.save(model.state_dict(), ckpt_path)
                for f in glob.glob(os.path.join(root_ckpt_dir, 'epoch_*.pt')):
                    nb = int(os.path.splitext(os.path.basename(f))[0].split('_')[-1])
                    if nb < epoch - 1:
                        os.remove(f)

                # Save the best direct-joint checkpoint.
                if val_metrics['loss'] < best_val_loss:
                    ...
                    torch.save({
                        ...
                        'tau': tau,
                        'keep_fraction': params['keep_fraction'],
                        'nominal_encoder_cbr': params['ratio'],
                        'actual_cbr': params['ratio'] * params['keep_fraction'],
                        'split_manifest_sha256': split_manifest_hash,
                    }, os.path.join(root_ckpt_dir, 'best.pt'))
                    print(f"\n  ✓ Best saved — "
        ...
        torch.save(model.state_dict(),
                   os.path.join(root_ckpt_dir, f'interrupted_epoch_{epoch}.pt'))

    # Final evaluation must use the selected checkpoint, not the last epoch.
    best_path = os.path.join(root_ckpt_dir, 'best.pt')
    if os.path.isfile(best_path):
        best_artifact = torch.load(
            best_path, map_location=device, weights_only=False
        )
        model.load_state_dict(best_artifact['model_state'], strict=True)
        model.set_temperature(best_artifact['tau'])
    model.set_selector_mode('learned')
    ...
    learned_rows = paired_validation_evaluation(
        model, device, val_loader, 'learned', params['val_channel_seed']
    )
    random_rows = paired_validation_evaluation(
        model, device, val_loader, 'random', params['val_channel_seed']
    )
    ...
    paired_summary = {
        ...
        'channel_seed': params['val_channel_seed'],
    }
    ...
    print(f"Exact keep fraction: {final_val['rate_mean']:.3f}")
    print(f"Actual CBR         : "
          f"{params['ratio'] * params['keep_fraction']:.6f}")
    ...
    with open(os.path.join(root_config_dir, 'config.yaml'), 'w') as f:
        yaml.dump({
            **{k: v for k, v in params.items() if k != 'device'},
            ...
            'topk_gradient_audit': topk_audit,
            ...
            'baseline_split_manifest_sha256': baseline_manifest_hash,
            'tsn_split_manifest_sha256': tsn_manifest_hash,
...
def main():
    ...
    params = {
        ...
        'val_channel_seed'   : args.val_channel_seed,
        'out_dir'            : args.out,
        'device'             : args.device,
