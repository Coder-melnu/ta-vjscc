...
def parser():
    ...
    p.add_argument(
        '--export_clip_ids', nargs='*', type=int, default=[],
        help='Clip IDs to export from the same deterministic evaluation pass',
    )
    return p.parse_args()
...
@torch.no_grad()
def evaluate(videojscc, tsn, loader, device, channel_seed, export_clip_ids=()):
    rows = []
    exports = {}
    export_clip_ids = set(export_clip_ids)
    ...
    with deterministic_channel(channel_seed, device):
        for gops, labels in tqdm(loader, desc='evaluation'):
            ...
            probabilities_recon = logits_recon.softmax(dim=1)
            probabilities_clean = logits_clean.softmax(dim=1)
            ...
            for item in range(batch):
                ...
                reconstructed_prediction = int(top5_recon[item, 0])
                clean_prediction = int(top5_clean[item, 0])
                rows.append({
                    'clip_id': clip_id, 'label': true_label,
                    ...
                    'reconstructed_predicted_label': reconstructed_prediction,
                    'reconstructed_true_class_probability': float(
                        probabilities_recon[item, true_label]
                    ),
                    'reconstructed_predicted_class_probability': float(
                        probabilities_recon[item, reconstructed_prediction]
                    ),
                    'clean_predicted_label': clean_prediction,
                    'clean_true_class_probability': float(
                        probabilities_clean[item, true_label]
                    ),
                    'clean_predicted_class_probability': float(
                        probabilities_clean[item, clean_prediction]
                    ),
                })
                if clip_id in export_clip_ids:
                    exports[clip_id] = {
                        'clean': gops[item].detach().cpu(),
                        'reconstructed': reconstructed[item].detach().cpu(),
                        'row': rows[-1].copy(),
                    }
                clip_id += 1
    missing = export_clip_ids - set(exports)
    if missing:
        raise IndexError(f'Export clip IDs not found: {sorted(missing)}')
    return rows, exports
...
def main():
    ...
    if (args.split == 'validation'
            and tsn_metadata.get('split_manifest_sha256') != manifest_hash):
        raise RuntimeError(
            'TSN checkpoint split manifest does not match the evaluation manifest'
        )
    ...
    rows, exports = evaluate(
        videojscc, tsn, loader, device, args.channel_seed,
        export_clip_ids=args.export_clip_ids,
    )
    ...
    if args.split == 'validation':
        manifest_items = manifest['fixed_validation_gops']
    else:
        manifest_items = manifest['samples']
    export_root = output / 'selected_clip_exports'
    for export_id, payload in sorted(exports.items()):
        row = payload['row']
        item = manifest_items[export_id]
        class_name = idx_to_class[row['label']]
        clip_dir = export_root / f"clip_{export_id:04d}_{class_name}"
        clip_dir.mkdir(parents=True, exist_ok=False)
        clean = payload['clean']
        reconstructed = payload['reconstructed']
        error_x5 = (clean - reconstructed).abs().mul(5).clamp(0, 1)
        save_image(
            make_grid(
                torch.cat((clean, reconstructed, error_x5), dim=0),
                nrow=clean.shape[0], padding=2,
            ),
            clip_dir / 'clean_reconstructed_error_x5.png',
        )
        for frame_index in range(clean.shape[0]):
            save_image(clean[frame_index], clip_dir / f'clean_{frame_index + 1:02d}.png')
            save_image(
                reconstructed[frame_index],
                clip_dir / f'reconstructed_{frame_index + 1:02d}.png',
            )
        metadata = {
            **row,
            'true_class': class_name,
            'reconstructed_predicted_class': idx_to_class[
                row['reconstructed_predicted_label']
            ],
            'clean_predicted_class': idx_to_class[row['clean_predicted_label']],
            'frame_dir': item['frame_dir'],
            'frame_files': item['frame_files'],
            'comparison_rows': ['clean', 'reconstructed', 'absolute_error_x5'],
        }
        (clip_dir / 'metadata.json').write_text(json.dumps(metadata, indent=2))
    evaluation_config = {
