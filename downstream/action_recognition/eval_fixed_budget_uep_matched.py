# -*- coding: utf-8 -*-
"""Matched validation/test evaluator for fixed-budget spatial UEP.
...
def parser():
    ...
    p.add_argument("--out", default="./out_fixed_budget_uep_matched")
    return p.parse_args()
...
def load_model(args, device, evaluation_manifest_hash):
    ...
    if not isinstance(artifact, dict) or "model_state" not in artifact:
        raise KeyError("Expected fixed-budget checkpoint with model_state")
    ...
    model = TAVideoJSCCFixedBudgetUEP(
        ...
        lambda_recon=1.0, high_fraction=args.high_fraction,
        low_power=args.low_power, tau=args.tau, random_seed=args.random_seed,
    )
...
def main():
    ...
    name = f"FixedBudgetUEP_{args.split}_{args.channel}_c{args.c}_snr{args.snr:g}_seed{args.channel_seed}"
