# -*- coding: utf-8 -*-
"""Matched TSN evaluation-only audit of genuine top-k latent transmission.
...
def load_selector_model(args, device, evaluation_manifest_hash):
    ...
    if tsn_manifest_hash != LOCKED_VALIDATION_MANIFEST:
        raise RuntimeError("TSN checkpoint does not match the locked training manifest")
    ...
    # Retain the locked frozen TSN already loaded inside TAVideoJSCC.
    return model, {
        ...
        "action_evaluator": "locked_tsn_resnet50",
        "tsn_checkpoint_sha256": sha256_file(args.tsn_ckpt),
        "tsn_checkpoint_metadata": model.tsn.checkpoint_metadata,
    }

    # Unreachable R(2+1)D reference retained from the validated source.
