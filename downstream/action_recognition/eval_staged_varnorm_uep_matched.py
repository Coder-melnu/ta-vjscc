# -*- coding: utf-8 -*-
"""Matched evaluator for staged variance-normalised spatial UEP.
...
def parser():
    ...
    p.add_argument("--out", default="./out_staged_varnorm_uep_matched")
    return p.parse_args()
...
def load_model(args, device, evaluation_manifest_hash):
    ...
    if artifact.get("method") != "staged_variance_normalised_continuous_spatial_uep":
        raise RuntimeError(
...
def main():
    ...
    name = (
        f"StagedVarNormUEP_{args.split}_{args.channel}"
