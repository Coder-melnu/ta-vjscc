# -*- coding: utf-8 -*-
"""Train the fixed-temperature, fixed-budget spatial UEP pilot.

All symbols are transmitted.  Half the spatial locations per encoded frame get
relative power 1.5 and half get 0.5; exact renormalisation preserves total
channel-input energy.  A straight-through top-k gate trains importance
placement.  Model selection uses only locked internal validation.
"""
...
from model.ta_video_jscc_uep_fixed_budget import TAVideoJSCCFixedBudgetUEP
...
def train_epoch(model, optimizer, device, loader, max_iters=None):
    ...
    totals = {
        ...
        "relative_power_cv": 0.0, "high_fraction": 0.0,
    }
    iterations = 0
...
def audit_fixed_budget(model, loader, device, channel_seed):
    """Verify power budget, neutral uniform path, gradients, and frozen TSN."""
    model.eval()
    ...
    def run(mode):
        ...
        model.reset_random_sequence()
        with fixed_rng(device, channel_seed):
            reconstructed, power, _ = model(gops)
        return reconstructed, power, dict(model.last_power_audit)

    bypass, _, bypass_audit = run("bypass")
    all_ones, _, all_ones_audit = run("all_ones")
    learned, learned_power, learned_audit = run("learned")
    ...
    fraction = model.allocator.high_fraction
    low = model.allocator.low_power
    high = model.allocator.high_power
    expected_std = (
        fraction * (high - 1.0) ** 2
        + (1.0 - fraction) * (low - 1.0) ** 2
    ) ** 0.5
    expected = {
        "mean": 1.0,
        "std": expected_std,
    ...
    observed = {
        ...
        "high_fraction": learned_audit["high_fraction_actual"],
    }
    for key in expected:
        if abs(observed[key] - expected[key]) > 1e-6:
            raise RuntimeError(
                f"Fixed budget failed for {key}: {observed[key]} != {expected[key]}"
            )
    if not learned_audit["all_symbols_transmitted"]:
        raise RuntimeError("Fixed-budget UEP dropped at least one symbol")
    if (
        abs(learned_audit["energy_ratio_min"] - 1.0) > 2e-6
        or abs(learned_audit["energy_ratio_max"] - 1.0) > 2e-6
    ):
        raise RuntimeError("Fixed-budget path violated the total-energy constraint")

    model.train(); model.tsn.eval(); model.set_allocation_mode("learned")
    ...
    codec_gradient_l1 = sum(
        ...
        if name.startswith(("jscc.", "temporal.")) and parameter.grad is not None
    )
    ...
    result = {
        ...
        "all_symbols_transmitted": learned_audit["all_symbols_transmitted"],
    ...
    print("\nFixed-budget UEP audit")
    print(json.dumps(result, indent=2))
    print("PASS: fixed unequal power, exact total energy, all symbols sent")
    return result
...
def parser():
    p = argparse.ArgumentParser(
        description="Train fixed-temperature fixed-budget spatial UEP"
    )
    ...
    p.add_argument("--low_power", type=float, default=0.5)
    ...
    p.add_argument("--out", default="./out_ta_uep_fixed_budget")
...
def train_pipeline(params):
    ...
    model = TAVideoJSCCFixedBudgetUEP(
        ...
        high_fraction=params["high_fraction"], low_power=params["low_power"],
        tau=params["tau"], random_seed=params["random_seed"],
    ).to(device)
    ...
    print(
        f"Fixed UEP: top {params['high_fraction']*100:.0f}% high power | "
        f"low={model.allocator.low_power:.3f} | high={model.allocator.high_power:.3f} | "
        f"tau={model.allocator.tau:.3f}"
    )
    audit = audit_fixed_budget(
    ...
    tag = (
        f"TAUEPFixed_{params['channel']}_c{c}_snr{params['snr']}"
        f"_ratio{params['ratio']:.4f}_top{params['high_fraction']:.2f}"
        f"_plow{params['low_power']:.2f}_tau{params['tau']:.2f}"
    ...
    fields = [
        ...
        "val_psnr", "power_mean", "power_std", "power_cv", "high_fraction",
    ]
    ...
    metadata = {
        "method": "fixed_budget_spatial_uep",
        ...
        "low_power": model.allocator.low_power,
        "high_power": model.allocator.high_power,
        "tau": model.allocator.tau,
    }
        ...
        with tqdm(range(params["epochs"]), disable=params["disable_tqdm"]) as progress:
            for index in progress:
                ...
                metrics_writer.writerow({
                    ...
                    "high_fraction": val["high_fraction"],
