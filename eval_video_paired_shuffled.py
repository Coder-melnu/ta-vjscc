...
OUTPUT_DIR = 'week3_results_paired_shuffled_snr13'#'week3_results'
DEVICE         = 'cuda:0'
...
def evaluate_checkpoint(
    ...
    for snr in tqdm(snr_list, desc=f"SNR sweep"):
		...
		# Decode once so both paths use identical channel noise
                ...
                # Fixed permutation disrupts temporal adjacency: [0,1,2,3,4]
                # becomes [0,2,4,1,3].
                perm = torch.tensor(
                    [0, 2, 4, 1, 3],
                    device=out_notmp_raw.device
                )
                inverse_perm = torch.argsort(perm)

                shuffled = out_notmp_raw[:, perm]
                out_temp_raw = model.temporal(shuffled)[:, inverse_perm]
