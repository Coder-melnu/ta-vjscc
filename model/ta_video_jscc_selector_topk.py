...
class TAVideoJSCC(nn.Module):
    ...
    def _legacy_soft_forward(self, x: torch.Tensor) -> tuple:
        """
    ...
    def _topk_mask(self, logits):
        """Exact top-k forward mask with a budget-normalised STE gradient."""
        flat_logits = logits.flatten(1)
        count, locations = flat_logits.shape
        keep_count = max(1, min(
            locations, round(locations * self.keep_fraction)
        ))
        ...
        if self.selector_mode == 'random':
            ...
            for row in range(count):
                ...
                generator.manual_seed(self.random_mask_seed + row)
                indices = torch.randperm(
                    locations, device=logits.device, generator=generator
                )[:keep_count]
                hard = torch.zeros(
                    locations, device=logits.device, dtype=logits.dtype
                )
                hard.scatter_(0, indices, 1.0)
                hard_rows.append(hard)
            hard_flat = torch.stack(hard_rows)
        else:
            indices = flat_logits.topk(
                keep_count, dim=1, largest=True, sorted=False
            ).indices
            hard_flat = torch.zeros_like(flat_logits)
            hard_flat.scatter_(1, indices, 1.0)

        soft_flat = torch.sigmoid(2.0 * flat_logits / self.selector.tau)
        soft_flat = (
            soft_flat * (keep_count / locations)
            / soft_flat.mean(dim=1, keepdim=True).clamp_min(1e-6)
        )
        ste_flat = hard_flat + soft_flat - soft_flat.detach()
        shape = (count, 1, logits.shape[2], logits.shape[3])
        return ste_flat.view(shape), hard_flat.view(shape), keep_count, locations

    def forward(self, x: torch.Tensor) -> tuple:
        """Transmit exactly top-k spatial locations and zero-fill the rest."""
        B, N, C, H, W = x.shape
        if N != self.n_frames:
            raise ValueError(f'Expected {self.n_frames} frames, got {N}')
        x_flat = x.reshape(B * N, C, H, W)
        z = self.jscc.encoder(x_flat)

        if self.selector_mode in {'bypass', 'all_ones'}:
            mask = torch.ones(
                B * N, 1, z.shape[2], z.shape[3],
                device=z.device, dtype=z.dtype,
            )
            logits = torch.zeros_like(mask)
            z_rx = self.jscc.channel(z) if self.jscc.channel is not None else z
            keep_count = z.shape[2] * z.shape[3]
            locations = keep_count
            hard_mask = mask
        else:
            if self.jscc.channel is None:
                raise RuntimeError('Top-k training requires an active channel')
            if self.jscc.channel.channel_type != 'AWGN':
                raise NotImplementedError(
                    'This audited top-k trainer currently supports AWGN only'
                )
            logits = self.selector.scorer(z)
            mask, hard_mask, keep_count, locations = self._topk_mask(logits)
            z_masked = z * mask
            dimensions = z.shape[1] * keep_count
            energy = z_masked.square().sum(
                dim=(1, 2, 3), keepdim=True
            ).clamp_min(1e-12)
            z_tx = math.sqrt(dimensions) * z_masked / energy.sqrt()
            snr_linear = 10.0 ** (self.jscc.channel.snr / 10.0)
            noise_std = math.sqrt(1.0 / (2.0 * snr_linear))
            noise = torch.randn_like(z_tx) * noise_std
            z_rx = z_tx + noise * hard_mask.detach()

        spatial_power = z_rx.detach().square().sum(dim=1, keepdim=True)
        self.last_spatial_power = spatial_power / spatial_power.flatten(1).sum(
            dim=1, keepdim=True
        ).view(-1, 1, 1, 1).clamp_min(1e-12)
        self.last_power_audit = {
            'mask_mean': hard_mask.detach().mean().item(),
            'keep_count': keep_count,
            'locations': locations,
            'actual_cbr_fraction': keep_count / locations,
        }
        x_hat = self.jscc.decoder(z_rx).reshape(B, N, C, H, W)
        return self.temporal(x_hat), mask, logits
