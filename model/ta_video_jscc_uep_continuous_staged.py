# -*- coding: utf-8 -*-
"""Task-aware VideoJSCC with staged variance-normalised spatial allocation.
...
class VarianceNormalisedContinuousSpatialUEP(nn.Module):
    """Assign continuous power from variance-normalised spatial scores."""
    ...
    def _standardise_scores(self, scores):
        # Standardise independently within every encoded frame.
        ...
        ).clamp_min(self.score_eps)
        return centred / score_scale

    def _scores_to_power(self, scores):
        spatial_dims = tuple(range(1, scores.ndim))
        standardised = self._standardise_scores(scores)

        # Alpha controls contrast; score magnitude cannot normally
        # disable allocation after variance normalisation.
        raw_power = 1.0 + self.alpha * torch.tanh(standardised)

        # Exact unit mean within every encoded frame.
...
class TAVideoJSCCStagedContinuousUEP(nn.Module):
    """Staged VideoJSCC with variance-normalised continuous UEP."""
    ...
    def __init__(
        ...
        self.allocator = VarianceNormalisedContinuousSpatialUEP(
            channels=2 * c,
            alpha=alpha,
            score_eps=score_eps,
