...
Extends VideoJSCC with:
    ...
    3. Frozen TSN or TSM for task loss — gradients flow through x_hat into
       decoder/encoder
...
Pipeline:
    ...
    → Frozen task model → logits (B, 101) [frozen weights; input gradient retained]
"""
...
class FrozenTSM(nn.Module):
    """Frozen five-frame TSM used for task loss and action evaluation.

    The checkpoint must be produced by ``finetune_tsm_split.py`` and contain
    the full ``TSMResNet50`` state dict.  All task-network parameters are
    frozen, but the forward pass is intentionally *not* enclosed in
    ``torch.no_grad()``.  Consequently, cross-entropy gradients can propagate
    from the fixed TSM through reconstructed frames into the TA-VideoJSCC
    decoder, temporal module, selector, and encoder.

    Calling ``TAVideoJSCC.train()`` must not activate TSM dropout or update its
    BatchNorm statistics.  This wrapper therefore overrides ``train()`` and
    remains permanently in evaluation mode.
    """
    ...
    def __init__(
        ...
        from downstream.action_recognition.models.tsm_recognizer import TSMResNet50

        if not checkpoint_path or not os.path.isfile(checkpoint_path):
            raise FileNotFoundError(
                f"TSM UCF101 checkpoint not found: {checkpoint_path}"
            )
        ...
        if 'model_state_dict' not in checkpoint:
            raise KeyError(
                "Expected a fine-tuned TSM checkpoint containing "
                "'model_state_dict'. Do not pass the raw Kinetics checkpoint."
            )

        checkpoint_segments = int(checkpoint.get('num_segments', num_segments))
        checkpoint_classes = int(checkpoint.get('class_count', 101))
        if checkpoint_segments != num_segments:
            raise ValueError(
                f"TSM checkpoint uses {checkpoint_segments} segments, but "
                f"TA-VideoJSCC uses {num_segments}."
            )
        if checkpoint_classes != 101:
            raise ValueError(
                f"Expected 101 UCF101 classes, found {checkpoint_classes}."
            )

        self.model = TSMResNet50(
            num_classes=101,
        ...
        print(
            f"[FrozenTSM] Loaded {checkpoint_path} | "
            f"selected epoch={checkpoint.get('epoch', 'unknown')} | "
            f"internal-val Top-1={checkpoint.get('val_top1', 'unknown')}"
        )
    ...
    def forward(self, frames: torch.Tensor) -> torch.Tensor:
        """Return UCF101 logits while retaining gradients to ``frames``."""
        if frames.ndim != 5:
            raise ValueError(
...
class TAVideoJSCC(nn.Module):
    ...
    Args:
        ...
        task_model     : 'tsm' (recommended), 'tsn' (historical), or 'none'
        tsm_ckpt       : full five-frame UCF101 TSM checkpoint
        tsn_head_ckpt  : historical Linear(2048,101) TSN head checkpoint
    ...
    def __init__(
        ...
        # Frozen downstream task model.  Keep the attribute name ``tsn`` for
        # compatibility with the existing training loop, which calls
        # ``model.tsn.eval()`` after placing the full TA model in train mode.
        task_model = task_model.lower()
        if task_model == 'tsm':
            if tsm_ckpt is None:
                raise ValueError("task_model='tsm' requires --tsm_ckpt")
            self.tsn = FrozenTSM(
                checkpoint_path=tsm_ckpt,
                num_segments=n_frames,
                device=device,
            )
        elif task_model == 'tsn':
            if tsn_head_ckpt is None:
                raise ValueError("task_model='tsn' requires --tsn_head_ckpt")
            self.tsn = FrozenTSN(head_ckpt=tsn_head_ckpt, device=device)
        elif task_model == 'none':
            self.tsn = None
            print("[TAVideoJSCC] No task model — task loss will be zero")
        else:
            raise ValueError(
                f"task_model must be 'tsm', 'tsn', or 'none'; got {task_model}"
            )
